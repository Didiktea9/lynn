...
from LynnRoBot.modules import ALL_MODULES
from LynnRoBot.modules.helper_funcs.chat_status import is_user_admin
from LynnRoBot.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("LynnRoBot.modules." + module_name)
...
@run_async
def Lynn_about_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "lynn_":
        query.message.edit_text(
 ...
 ❍ I check for admins' permissions before executing any command and more stuffs
    ...
    If you have any question about Lynn, let us know at @puituflynn.""",
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="Back", callback_data="lynn_back")
                 ]
                ]
            ),
        )
    elif query.data == "lynn_back":
        query.message.edit_text(
                PM_START_TEXT,
                reply_markup=InlineKeyboardMarkup(buttons),
                parse_mode=ParseMode.MARKDOWN,
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(Lynn_about_callback, pattern=r"lynn_")
